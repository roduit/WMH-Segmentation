...
def compute_neig_features(qmap: np.ndarray, window_size: int, map_names: list) -> tuple:
    ...
    Args:
        qmap (np.ndarray): The MRI images.
        window_size (int): Size of the window for neighborhood features.
    ...
    """
    # ---- Compute neighborhood features for each map separately ----
    feats = []
    added_feature_names = []
...
def compute_log_features(
    qmap: np.ndarray, map_names: list, sigma: int
    ...
    # ---- Apply Laplacian of Gaussian to each map separately ----
    for c in range(qmap.shape[-1]):
...
def lbp_3d(volume: np.ndarray, radius: int = 1) -> np.ndarray:
    ...
    The LBP is computed for a connectivity 3, meaning with 26 neighbours.
...
def compute_wm_energy(
    ...
    # ---- Erode the WM mask to exlude border values between GM and WM ----
    ...
    for c in range(qmaps.shape[-1]):
        ...
        if wm_values.size > 0:
            # ---- Compute 5th and 95th perc. and cap values inside [0.05,0.95]----
